...
Sémantique complète façon C (Option B) implémentée et vérifiée :
- **AST** : `Expr::IncDec { op: IncDecOp, target: Box<Expr>, span }` (`src/parsing/ast.d/expressions.rs`), `IncDecOp::{PreInc,PreDec,PostInc,PostDec}`.
- **Lexer** (`src/parsing/lexer.d/tokenizer.d/next_token.rs`) : nouveaux tokens `PlusPlus`/`MinusMinus` (lookahead sur `+`/`-`, même schéma que `==`/`<=`).
- **Parser** (`src/parsing/parser.d/expressions.rs`) : préfixe dans `parse_unary` (récursif comme `-`/`not`), suffixe dans `parse_postfix`. **Point non anticipé par la conception initiale, corrigé pendant l'implémentation** : le suffixe `expr++`/`expr--` doit être restreint à la MÊME LIGNE que la fin de `expr` (`Parser::previous_line`, `primitives.rs`) — sans cette restriction, `var c:Counter = use Counter(100)` suivi, sur la ligne SUIVANTE, de `++c.value` (nouvelle instruction préfixe) se faisait avaler comme `(use Counter(100))++` puis `c.value` orphelin, ce langage n'ayant pas de point-virgule. Même restriction que JavaScript impose à `++`/`--` postfixe (ASI, "no LineTerminator here") pour exactement la même raison. Confirmé par reproduction puis test dédié (`prefixOnNextLineNotSwallowedTest`).
- **Sema** (`src/sema/typecheck.rs`, cas `Expr::IncDec` dans `infer_expr`) : même validation de forme que `Stmt::Assign` (`Ident`/`Field`/`Index` uniquement, `Ident` doit être mutable) + validation de type (`int`/`float` uniquement, nouvelle erreur `SemaError::IncDecInvalidType`). Un seul appel à `infer_expr(target)` (jamais deux, pour ne pas déclencher deux fois `use_binding` sur un `Ident` ou dupliquer les erreurs internes de `Field`/`Index`).
- **Lowering** (`lower_incdec`, `src/lower/stmt.d/statements.d/assignments.rs`, appelée depuis `Expr::IncDec` dans `lower.rs`) : charge l'ancienne valeur, calcule `± 1`, stocke, retourne l'ancienne (suffixe) ou la nouvelle (préfixe) — `object`/`index` évalués UNE SEULE fois (vérifié par test dédié, `singleEvaluationOfIndexTest` : `arr[bump()]++` n'appelle `bump()` qu'une fois).
- **Refactor de duplication découvert en cours de route** : la détection map-vs-array pour `Expr::Index` était dupliquée entre la lecture (`lower.rs`) et l'écriture (`lower_assign`, `assignments.rs`) — factorisée dans `is_map_target` (`src/lower/expr.d/helpers.rs`), réutilisée aussi par `lower_incdec`. Même chose pour le calcul de l'offset d'un champ (`lower_assign` avait sa propre logique inline au lieu d'appeler `field_offset`/`field_ir_type` déjà existants) — convergé vers les fonctions partagées.
- **Tests** : `examples/33_increment_decrement.oc` (exemple de démonstration, toutes les formes) ; `examples/tests/45_increment_decrementTest.oc` (27 assertions : pré/post sur int/float, cible `Ident`/`Field`/`Index`, usage en expression, unicité d'évaluation, non-régression du décalage de ligne, non-régression de la double négation `- -x`) ; deux nouvelles erreurs volontaires ajoutées à `examples/21_errors.oc` (cible invalide, type invalide).
- **Documentation** : `docs/EBNF.md` §10/§11 (et son doublon §31) mis à jour — nouvelles productions `UnaryExpr`/`PostfixExpr`, note sur la restriction de ligne et l'effet de bord sur la double négation.
- **Aucune mise à jour nécessaire** : extension VSCode (le regex `[+\-*/%]` du TextMate grammar colore déjà `++`/`--` correctement, de façon incidente, comme deux caractères d'opérateur adjacents) ; `ocaracs`/`ocaraunit` (aucun lexer propre, invoquent le binaire `ocara` compilé).
- `make regression` : 637 PASS, 0 FAIL, 0 ERREUR (610 avant ce chantier + 27 nouvelles assertions).
