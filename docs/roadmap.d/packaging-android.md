# Support Android — infrastructure de cross-compilation établie (packaging APK restant)

**Mise à jour** : le constat initial ci-dessous ("aucune preuve", "aucune infrastructure") a motivé l'ouverture de ce ticket, mais ne reflète plus l'état actuel — voir les sous-chantiers plus bas, **les 4 sont maintenant faits et vérifiés** (NDK r30 installé, `.o`/`.a`/`.so` AArch64 réels produits et inspectés, y compris un `.so` complet avec SDL3+image+ttf+mixer). Conservé tel quel comme trace de l'analyse initiale.
...
### 4. Backend Android pour SDL3 — **Fait, vérifié** (essai réel de cross-compilation)

**Terminé.** Contrairement aux sous-chantiers 1-3 (largement déductibles par lecture de code), celui-ci a nécessité un vrai essai de cross-compilation — deux bugs concrets ont été trouvés et corrigés, pas seulement des lectures de documentation :

- Nouvelle cible `make build-runtime-sdl-android` (`Makefile`), même patron que `build-runtime-android` (§3) : `ocara_runtime_sdl` (SDL3 + image/ttf/mixer, feature `build-from-source-static`, cmake) cross-compilé pour `aarch64-linux-android`. `sdl3-sys` gère déjà lui-même les défines cmake spécifiques Android (`ANDROID_ABI`, `CMAKE_SYSTEM_NAME`, voir `sdl3-sys/build-common.rs`) — il manquait seulement `CMAKE_TOOLCHAIN_FILE` (pointé vers `$NDK/build/cmake/android.toolchain.cmake`), sans quoi cmake ignore silencieusement ces défines et vise l'hôte.
- **Bug réel #1** : le codec MOD/tracker vendored de SDL_mixer (`libxmp`) échoue à l'édition de liens pour Android — sa variante partagée par défaut (`SDLMIXER_MOD_XMP_SHARED`) produit un script de version (`.map`) qui référence des symboles (`xmp_set_player`/`xmp_get_player`) absents du binaire réellement compilé pour cette cible ; `ld.lld` (linker du NDK) est plus strict que le `ld` GNU habituel sur ce point et refuse de lier. Forcer la variante statique (`no-sdlmixer-mod-xmp-shared`) contourne ce problème mais en révèle un second, plus profond : le `.pc` généré par cmake pour la config statique contient une expression génératrice CMake non résolue (`$<TARGET_FILE_BASE_NAME:...>`), que `rpkg-config` (utilisé par `sdl3-mixer-sys`) prend alors littéralement pour un nom de bibliothèque — erreur de compilation Rust. **Résolu** en désactivant entièrement ce codec de niche pour Android (`sdl3-mixer-sys` avec la feature `no-sdlmixer-mod-xmp` ajoutée dans `runtime_sdl/Cargo.toml`) plutôt que de continuer à contourner des bugs amont de CMake/rpkg-config — WAV/OGG/MP3/FLAC/Opus restent disponibles, seul le format "tracker" (Amiga/DOS) est perdu, et seulement pour Android (aucun exemple/doc existant n'en dépend, vérifié par recherche dans le dépôt). **Aucun impact sur le build hôte** (vérifié : `make build`/`make tests`/`make regression` toujours 100% verts, y compris l'exemple `game_sdl` qui ouvre réellement une fenêtre SDL et tourne 180 frames).
- **Bug réel #2** : au moins un codec de SDL_mixer (`gme`, chiptune) est en C++ — sans `CXX_aarch64_linux_android` explicite, `cc`-rs devine `aarch64-linux-android-clang++` (même piège de nommage binutils que `RANLIB`, §3) et échoue. Corrigé en fixant `CXX_aarch64_linux_android` vers `<clang>++` dans la cible Make.
- `link_android` (`src/codegen/link.rs`) accepte désormais un `libocara_runtime_sdl.a` optionnel (nouveau flag `--android-runtime-sdl`), requis seulement si le programme importe `ocara.SDL` (`needs_sdl`) — le rejet explicite pré-existant pour SDL est resté en place pour le cas où ce flag n'est PAS fourni (jamais de `.so` silencieusement incomplet).
- **Bibliothèques système NDK requises, déterminées empiriquement** (`nm -D --undefined-only` sur un premier `.so` sans elles, pas devinées à l'avance) : `-landroid` (assets APK, `ALooper`, `ANativeWindow`, capteurs), `-llog` (logs système), `-lGLESv2` (rendu OpenGL ES 2, backend graphique de SDL), `-lOpenSLES` (backend audio), `-lc++` (runtime C++ — résolu par le NDK vers `libc++_shared.so`, **doit être copié dans `jniLibs/<abi>/` de l'APK final**, à côté du `.so` Ocara — pattern standard NDK, Google déconseille explicitement de lier libc++ statiquement dans plusieurs `.so` d'un même processus).

#### Vérification

`ocara sdl_hello.oc --target aarch64-linux-android --android-runtime libocara_runtime.a --android-runtime-sdl libocara_runtime_sdl.a --android-ndk <NDK> -o libmain.so` (programme important `ocara.SDL`, créant une fenêtre) produit un `.so` vérifié avec la même rigueur que le sous-chantier 2 :

```
$ readelf -d libmain.so | grep -i textrel
(rien — toujours pas de DT_TEXTREL malgré la taille bien plus importante du binaire)
$ readelf -d libmain.so | grep NEEDED
 NEEDED  libm.so  libz.so  libandroid.so  liblog.so  libGLESv2.so  libOpenSLES.so  libc++_shared.so  libdl.so  libc.so
```

Chaque groupe de symboles non résolus localement (`AAsset_*`/`ALooper_*`/`ANativeWindow_*`/`ASensor*` → `libandroid.so`, `__android_log_*` → `liblog.so`, `gl*` → `libGLESv2.so`, `slCreateEngine`/`SL_IID_*` → `libOpenSLES.so`, `__cxa_*`/`_Zd*`/`_Zn*`/`_ZSt9terminatev` → `libc++_shared.so`) a été confirmé comme réellement exporté par la bibliothèque système correspondante du sysroot NDK (`nm -D` sur chaque `.so` du NDK directement, pas seulement supposé).

`make build` : 0 warning. `make tests` : 101 passed. `make regression` (cache vidé) : 684 + 50 PASS, 0 FAIL.

**Non couvert, comme pour les sous-chantiers précédents** : exécution réelle sur un appareil/émulateur Android (JNI, ouverture effective d'une fenêtre/surface SDL) — nécessite un device ou un émulateur configuré, hors de portée de cet environnement. ABI autre que `aarch64` — non testées.

## Ampleur

**Les 4 sous-chantiers sont faits et vérifiés** (NDK r30 installé dans cet environnement, essais réels effectués, pas seulement de la lecture de code). Il reste le packaging final proprement dit (gabarit APK + injection du/des `.so` dans `jniLibs/`, potentiellement léger si le template Android officiel de SDL est réutilisé tel quel — voir la section stratégie ci-dessus) — non commencé. Ce qui reste non vérifiable dans cet environnement, quel que soit l'état du code : le comportement réel d'un `.so` une fois chargé sur un appareil/émulateur Android (JNI, `System.loadLibrary`, rendu SDL effectif) — nécessite un device réel ou un émulateur configuré. Toujours *(Massif)* en ampleur globale historique, mais l'infrastructure de compilation croisée elle-même (les 4 sous-chantiers) est désormais entièrement close ; ce qui reste (packaging APK, puis les chantiers voisins [webview hybride](packaging-android-webview-hybrid.md) et [GUI native](packaging-android-gui-native.md)) est du terrain neuf, pas une suite de ce même travail.

## Fichiers clés

`src/codegen/emit.d/emitter.rs` (sélection de cible + `is_pic` pour cible croisée — **fait**), `src/core/cli.rs` (flags `--target`/`--android-runtime`/`--android-runtime-sdl`/`--android-ndk` — **fait**), `Cargo.toml` racine (feature `arm64` de `cranelift-codegen` — **fait**), `src/codegen/link.rs` (`link_android`, lien final Android + SDL — **fait**), `Makefile` (cibles `build-runtime-android`/`build-runtime-sdl-android` — **fait**), `runtime/src/mutex.rs` (compilé et vérifié pour Bionic — **fait**), `runtime_sdl/Cargo.toml` (dépendance `sdl3`/`sdl3-mixer-sys`, feature `no-sdlmixer-mod-xmp` — **fait**), `runtime_tauri/` (hors périmètre Android, définitif), [securite-pie-cranelift-is-pic](securite-pie-cranelift-is-pic.md) (ticket voisin, note ajoutée sur `is_pic`, resté non traité pour le chemin hôte), [packaging-android-webview-hybrid](packaging-android-webview-hybrid.md) et [packaging-android-gui-native](packaging-android-gui-native.md) (chantiers suivants, packaging/UI).
