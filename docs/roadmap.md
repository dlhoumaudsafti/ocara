...
**La section "Priorité Haute" a été vide un temps** (représentation `mixed`, duplication statique/sucre du type des paramètres, couverture de tests Rust unitaires sur l'analyse d'échappement/ownership/boxing, recapture d'une fermeture imbriquée, SEGFAULT sur une fermeture créée dans un bloc `if`/`switch`, re-promotion d'une fermeture à chaque itération d'une boucle, dette `setjmp`/`longjmp`, race condition de `HTTPServer` — voir [memoire-boxing-durcissement](roadmap.d/memoire-boxing-durcissement.md), [qualite-parite-sucre-statique-param-types](roadmap.d/qualite-parite-sucre-statique-param-types.md), [qualite-tests-unitaires-critiques](roadmap.d/qualite-tests-unitaires-critiques.md), [langage-nested-closure-recapture](roadmap.d/langage-nested-closure-recapture.md), [langage-closure-promotion-block-scope](roadmap.d/langage-closure-promotion-block-scope.md), [langage-closure-promotion-in-loop](roadmap.d/langage-closure-promotion-in-loop.md), [exceptions-setjmp-longjmp-dette](roadmap.d/exceptions-setjmp-longjmp-dette.md) et [runtime-httpserver-race-condition](roadmap.d/runtime-httpserver-race-condition.md), toutes closes) — mais un nouveau point vient d'y être découvert, voir ci-dessous. Les sections Moyenne/Basse/Très Basse ci-dessous restent à traiter mais ne bloquent pas cette définition — voir la Légende.
...
- **`use Classe(...).méthode()` chaîné en une seule expression perd la valeur de retour de `méthode()` (retourne toujours `0`)** — compile sans la moindre erreur ni avertissement, résultat silencieusement faux ; affecte potentiellement `examples/advanced/tauri_httpserver/controllers/HomeController.oc` (`scoped visits:int = use Database().recordVisit()`). Découvert en écrivant le test de régression du destructeur de champ ressource (voir plus bas), sans aucun rapport avec ce chantier-là. *(Complexité non évaluée — cause racine non confirmée)* → [détails](roadmap.d/langage-use-chaine-valeur-retour-perdue.md)
