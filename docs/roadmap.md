...
- 🔴 **Découverte non corrigée, plus grave que prévu** : une `scoped`/`consumed` déclarée directement dans un bloc `main`/`init`/`error`/`success`/`exit` (pas dans un `if` imbriqué) n'est **jamais** libérée — le lowering de ces blocs court-circuite entièrement le mécanisme de libération. Nouvel item Haute priorité (voir ci-dessous).
...
- **`scoped`/`consumed` déclarée directement dans un bloc `main`/`init`/`error`/`success`/`exit` n'est jamais libérée** — le lowering de ces blocs ne passe pas par le mécanisme de libération normal ; touche potentiellement la plupart des programmes réels (`main{}` est quasi systématique). *(Structurel)* → [détails](roadmap.d/memoire-runtime-block-jamais-libere.md)
