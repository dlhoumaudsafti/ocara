...
(Deux points qui se trouvaient ici sont clos : la convention de nommage — décidée dans [docs/conventions.md](conventions.md) et portée par `ocaracs` (règles R07/R08/R09/R12), voir [qualite-convention-nommage-methodes](roadmap.d/qualite-convention-nommage-methodes.md) — et la concaténation de chaîne corrompue sur appareil Android réel, dont la cause racine réelle (des pointeurs de tas tagués par Scudo, pas un bug de codegen comme d'abord supposé) a été corrigée par un unique appel `mallopt` dans le pont JNI et vérifiée en exécution réelle sur un vrai téléphone, voir [runtime-android-aarch64-pic-string-concat](roadmap.d/runtime-android-aarch64-pic-string-concat.md).)
