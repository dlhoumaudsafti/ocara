...
- **Applications Android hybrides (WebView + serveur HTTP Ocara embarqué)** — fondations faites et vérifiées : pont JNI (`runtime_android_jni`) + squelette Gradle réel (`packaging/android/`), APK qui compile et embarque un `.so` Ocara chargé via JNI. Reste : adapter `examples/advanced/mini_project` (utilise `ocara.Tauri`, incompatible Android tel quel), cycle de vie, test sur appareil réel. *(Massive)* → [détails](roadmap.d/packaging-android-webview-hybrid.md)
