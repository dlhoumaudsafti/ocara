...
Ce que ça couvre concrètement, une fois les trois points de cette section clos : le mécanisme d'exceptions ne fait plus fuir silencieusement mémoire/ressources sans qu'au minimum ce soit détecté à la compilation ; une décision consciente est prise sur la protection native de `HTTPServer` (ou son absence assumée, documentée et testée) ; et une fermeture créée dans un bloc `if`/`while`/`for` ne fait plus planter le programme dès qu'on relit après le bloc la variable qu'elle capture. (La représentation `mixed`, la duplication statique/sucre du type des paramètres, la couverture de tests Rust unitaires sur l'analyse d'échappement/ownership/boxing, et la recapture d'une fermeture imbriquée dans une fermeture ont déjà été traitées : voir [memoire-boxing-durcissement](roadmap.d/memoire-boxing-durcissement.md), [qualite-parite-sucre-statique-param-types](roadmap.d/qualite-parite-sucre-statique-param-types.md), [qualite-tests-unitaires-critiques](roadmap.d/qualite-tests-unitaires-critiques.md) et [langage-nested-closure-recapture](roadmap.d/langage-nested-closure-recapture.md), clos.)
...
- **SEGFAULT : une fermeture créée dans un bloc `if`/`while`/`for` perd la promotion heap de la variable qu'elle capture, dès qu'on la relit après le bloc** — découvert en vérifiant le correctif de recapture ci-dessus. Aucune imbrication de fermeture requise (une fermeture à un seul niveau suffit) ; confirmé par `ocara --dump` : le code après le bloc lit la variable via un pointeur de cellule verrouillée qui n'a jamais été promu à cet endroit — accès mémoire invalide. Patron de code très courant, pas un cas exotique. *(Dangereuse)* → [détails](roadmap.d/langage-closure-promotion-block-scope.md)
