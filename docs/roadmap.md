...
_Dernière mise à jour : 2026-09-19_
...
**La section "Priorité Haute" a de nouveau un point ouvert** (voir ci-dessous, volontairement laissé de côté pour l'instant — voir sa fiche) — au sens de cette définition, le langage n'est momentanément pas "stable". De nombreux points l'ont déjà été et le resteront quoi qu'il arrive : la représentation `mixed`, la duplication statique/sucre du type des paramètres, la couverture de tests Rust unitaires sur l'analyse d'échappement/ownership/boxing, la recapture d'une fermeture imbriquée, le SEGFAULT sur une fermeture créée dans un bloc `if`/`switch`, la re-promotion d'une fermeture à chaque itération d'une boucle, la dette `setjmp`/`longjmp`, la race condition de `HTTPServer`, `use Classe(...).méthode()`/`HTTPRequest::méthode(...).méthode()` chaînés, `import Classe as Alias` (classe utilisateur) cassant la résolution de méthode héritée, et chaîner un appel de méthode sur le résultat `void` d'un appel précédent ont toutes été traitées : voir [memoire-boxing-durcissement](roadmap.d/memoire-boxing-durcissement.md), [qualite-parite-sucre-statique-param-types](roadmap.d/qualite-parite-sucre-statique-param-types.md), [qualite-tests-unitaires-critiques](roadmap.d/qualite-tests-unitaires-critiques.md), [langage-nested-closure-recapture](roadmap.d/langage-nested-closure-recapture.md), [langage-closure-promotion-block-scope](roadmap.d/langage-closure-promotion-block-scope.md), [langage-closure-promotion-in-loop](roadmap.d/langage-closure-promotion-in-loop.md), [exceptions-setjmp-longjmp-dette](roadmap.d/exceptions-setjmp-longjmp-dette.md), [runtime-httpserver-race-condition](roadmap.d/runtime-httpserver-race-condition.md), [langage-use-chaine-valeur-retour-perdue](roadmap.d/langage-use-chaine-valeur-retour-perdue.md), [langage-alias-classe-utilisateur-heritage-casse](roadmap.d/langage-alias-classe-utilisateur-heritage-casse.md) et [langage-appel-methode-sur-void-accepte](roadmap.d/langage-appel-methode-sur-void-accepte.md), toutes closes. Les sections Moyenne/Basse/Très Basse ci-dessous restent à traiter mais ne bloquent pas cette définition — voir la Légende.
...
- **Applications Android hybrides (WebView + serveur HTTP Ocara embarqué)** — fondations faites et vérifiées (pont JNI + squelette Gradle réel), `examples/advanced/mini_project` adapté pour Android (point d'entrée unique `main.oc`, multi-plateforme, `ocara.Tauri` compilé en talon no-op sur cette cible, voir ci-dessous). **Fonctionne bout en bout, vérifié en conditions réelles, sur un émulateur x86_64 accéléré (KVM) ET sur un vrai téléphone Android** : crash au démarrage, accès `public/`, redirection WebView externe et rendu HTML corrompu (pointeurs de tas Android tagués, voir ci-dessous) tous corrigés — `curl` sur l'appareil réel répond `HTTP 200, 2306 octets`, identique à x86_64. *(Massive)* → [détails](roadmap.d/packaging-android-webview-hybrid.md)
- **Concaténation corrompue sur appareil Android réel — cause racine réelle : pointeurs de tas tagués par Scudo, pas du codegen** — `Convert::intToStr(...)`/`System::OS`/toute chaîne issue d'un `alloc_str()` runtime produisait un grand nombre au lieu du contenu réel une fois concaténée, uniquement sur arm64 réel. Une première hypothèse de codegen Cranelift AArch64+PIC était fausse : la vraie cause est que Bionic/Scudo (depuis Android 11) tague l'octet de poids fort des pointeurs de tas, que `ocara_runtime` classait alors à tort comme un entier brut. **Corrigé** par un appel `mallopt(M_BIONIC_SET_HEAP_TAGGING_LEVEL, ...)` dans le pont JNI (désactive le tagging pour tout le processus) — vérifié en exécution réelle sur appareil physique. *(Complexité : contenue, un seul appel dans le pont JNI)* → [détails](roadmap.d/runtime-android-aarch64-pic-string-concat.md)
