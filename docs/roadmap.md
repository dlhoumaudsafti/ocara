...
- **Applications Android hybrides (WebView + serveur HTTP Ocara embarqué)** — faire tourner un programme du style `examples/advanced/mini_project` sur Android via une WebView pointée sur le serveur HTTP natif embarqué. Étape "pour commencer" voulue avant la GUI native (point suivant). *(Massive)* → [détails](roadmap.d/packaging-android-webview-hybrid.md)
