...
Ce que ça couvre concrètement, une fois les trois points de cette section clos : le mécanisme d'exceptions ne fait plus fuir silencieusement mémoire/ressources sans qu'au minimum ce soit détecté à la compilation ; une décision consciente est prise sur la protection native de `HTTPServer` (ou son absence assumée, documentée et testée) ; et une fermeture créée à l'intérieur d'une boucle voit correctement l'état accumulé des itérations précédentes, pas seulement la dernière. (La représentation `mixed`, la duplication statique/sucre du type des paramètres, la couverture de tests Rust unitaires sur l'analyse d'échappement/ownership/boxing, la recapture d'une fermeture imbriquée dans une fermeture, et le SEGFAULT sur une fermeture créée dans un bloc `if`/`switch` ont déjà été traitées : voir [memoire-boxing-durcissement](roadmap.d/memoire-boxing-durcissement.md), [qualite-parite-sucre-statique-param-types](roadmap.d/qualite-parite-sucre-statique-param-types.md), [qualite-tests-unitaires-critiques](roadmap.d/qualite-tests-unitaires-critiques.md), [langage-nested-closure-recapture](roadmap.d/langage-nested-closure-recapture.md) et [langage-closure-promotion-block-scope](roadmap.d/langage-closure-promotion-block-scope.md), clos.)
...
- **Une fermeture créée À L'INTÉRIEUR d'une boucle se re-promeut à chaque itération au lieu d'une seule fois** — sous-cas résiduel du SEGFAULT `if`/`switch` corrigé ci-dessus (root cause différente, la boucle ne bloquait déjà plus, mais ne voit que la dernière itération, pas l'état accumulé). Confirmé par `ocara --dump` : la séquence de promotion (allocation de cellule + copie de l'état) est dans le corps de boucle, ré-exécutée à chaque itération, toujours réinitialisée depuis le slot stack d'origine jamais mis à jour. Patron d'usage réaliste (accumulateur/compteur incrémenté par un callback à l'intérieur d'une boucle). *(Dangereuse)* → [détails](roadmap.d/langage-closure-promotion-in-loop.md)
