...
- **Applications Android hybrides (WebView + serveur HTTP Ocara embarqué)** — fondations faites et vérifiées (pont JNI + squelette Gradle réel), `examples/advanced/mini_project` adapté pour Android (`main_android.oc`, sans Tauri), **fonctionne bout en bout sur un émulateur x86_64 accéléré (KVM)**. **Ne fonctionne PAS encore sur un appareil physique réel** (testé sur un vrai téléphone Android : plante de façon reproductible — voir le nouveau ticket [runtime-android-exit-unsafe](roadmap.d/runtime-android-exit-unsafe.md), `__ocara_fail` appelle `std::process::exit()`, sûr sur desktop mais fatal sur Android). *(Massive)* → [détails](roadmap.d/packaging-android-webview-hybrid.md)
- **`__ocara_fail` dangereux sur Android** (`std::process::exit()` sur exception non gérée fait planter le processus — détruit des objets globaux d'ART/`libhwui` jamais conçus pour l'être en cours de vie d'une app) — découvert en testant le point précédent sur un vrai téléphone. Cause racine du déclencheur (échec `SQLite::open()`) non déterminée : aucune visibilité sur les logs du runtime Ocara depuis un `.so` Android. *(Complexité non évaluée)* → [détails](roadmap.d/runtime-android-exit-unsafe.md)
