...
fn generate_runtime_main(
    ...
    let has_error = program.runtime_blocks.iter().any(|b| b.kind == crate::ast::RuntimeBlockKind::Error);
    let has_exit = program.runtime_blocks.iter().any(|b| b.kind == crate::ast::RuntimeBlockKind::Exit);
    
    // Injecter les variables magiques au début si nécessaire
    if has_error || has_exit {
        // var ERROR:int = 0
        all_stmts.push(crate::ast::Stmt::Var {
            name: "ERROR".to_string(),
            ty: crate::ast::Type::Int,
            value: crate::ast::Expr::Literal(crate::ast::Literal::Int(0), Span::new(0, 0)),
            mutable: true,
            span: Span::new(0, 0),
        });
    }
    ...
    if has_exit {
        ...
        all_stmts.push(crate::ast::Stmt::Var {
            ...
            ty: crate::ast::Type::Bool,
            value: crate::ast::Expr::Literal(crate::ast::Literal::Bool(false), Span::new(0, 0)),
            mutable: true,
            span: Span::new(0, 0),
        });
    }
    ...
    for kind in &[crate::ast::RuntimeBlockKind::Init, crate::ast::RuntimeBlockKind::Main] {
    ...
    let has_success = program.runtime_blocks.iter().any(|b| b.kind == crate::ast::RuntimeBlockKind::Success);
    ...
    if has_error || has_success {
        ...
        if let Some(error_block) = program.runtime_blocks.iter().find(|b| b.kind == crate::ast::RuntimeBlockKind::Error) {
            if_body.extend(error_block.statements.clone());
        }
        
        // Dans le else, mettre SUCCESS = true puis les statements du bloc success
        if has_success {
            else_body.push(crate::ast::Stmt::Assign {
                target: crate::ast::Expr::Ident("SUCCESS".to_string(), Span::new(0, 0)),
                value: crate::ast::Expr::Literal(crate::ast::Literal::Bool(true), Span::new(0, 0)),
                span: Span::new(0, 0),
            });
            
            if let Some(success_block) = program.runtime_blocks.iter().find(|b| b.kind == crate::ast::RuntimeBlockKind::Success) {
                else_body.extend(success_block.statements.clone());
            }
        }
        
        // Créer le if statement : if (ERROR != 0)
        all_stmts.push(crate::ast::Stmt::If {
            condition: crate::ast::Expr::Binary {
                left: Box::new(crate::ast::Expr::Ident("ERROR".to_string(), Span::new(0, 0))),
                op: crate::ast::BinOp::NotEq,
                right: Box::new(crate::ast::Expr::Literal(crate::ast::Literal::Int(0), Span::new(0, 0))),
                span: Span::new(0, 0),
            },
    ...
    if let Some(exit_block) = program.runtime_blocks.iter().find(|b| b.kind == crate::ast::RuntimeBlockKind::Exit) {
        all_stmts.extend(exit_block.statements.clone());
    }
    
    // Retourner la valeur de ERROR (0 si succès, autre si erreur)
    all_stmts.push(crate::ast::Stmt::Return {
        value: Some(crate::ast::Expr::Ident("ERROR".to_string(), Span::new(0, 0))),
        span: Span::new(0, 0),
    });
    
    // Créer la fonction main() AST avec tous les statements mergés
    let main_func = crate::ast::FuncDecl {
        name: "main".to_string(),
        params: vec![],
...
pub fn lower_func(
    ...
    let updated_params: Vec<IrParam> = func.params.iter().enumerate().map(|(idx, param)| {
        ...
        if let crate::ast::Type::Array(inner) = &param.ty {
